...
import { armutQuiz } from '../../data/content';

export default function ArmutQuizPage() {
  ...
  const question = armutQuiz[currentQuestion];
  ...
  const nextQuestion = () => {
    if (currentQuestion < armutQuiz.length - 1) {
      setCurrentQuestion(prev => prev + 1);
      setSelectedAnswer(null);
      setIsAnswered(false);
    } else {
      setIsFinished(true);
      if (score >= armutQuiz.length - 2) {
  ...
  if (isFinished) {
    const percent = Math.round((score / armutQuiz.length) * 100);
    return (
      <div className="px-4 md:px-6 lg:px-8 py-4 max-w-2xl mx-auto text-center">
        ...
        <h1 className="text-2xl font-bold mb-2">{score} von {armutQuiz.length} richtig!</h1>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-left">
              ...
              <p className="text-white/90 text-sm">
                {percent >= 80 
                  ? "Super! Du verstehst den Unterschied zwischen absoluter und relativer Armut!" 
                  : percent >= 60
                  ? "Gut gemacht! Lies das Thema nochmal durch."
                  : "Nicht aufgeben! Schau dir die Unterschiede nochmal an."}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <Link href="/leni/sozialkunde/armut">
  ...
  return (
    <div className="px-4 md:px-6 lg:px-8 py-4 max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <Link href="/leni/sozialkunde/armut" className="flex items-center gap-2 text-gray-500 hover:text-gray-700">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <span className="font-medium text-gray-700">Frage {currentQuestion + 1}/{armutQuiz.length}</span>
      ...
      <div className="h-1.5 bg-gray-200 rounded-full mb-6 overflow-hidden">
        <div 
          ...
          style={{ width: `${((currentQuestion + 1) / armutQuiz.length) * 100}%` }}
        />
      </div>
      ...
      {isAnswered && (
        <button
          ...
          className="w-full py-4 bg-gradient-to-r from-orange-500 to-amber-600 text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:shadow-lg transition-shadow"
        >
          {currentQuestion < armutQuiz.length - 1 ? 'Weiter' : 'Ergebnis'}

...
import { gerechtigkeitQuiz } from '../../data/content';

export default function GerechtigkeitQuizPage() {
  ...
  const question = gerechtigkeitQuiz[currentQuestion];
  ...
  const nextQuestion = () => {
    if (currentQuestion < gerechtigkeitQuiz.length - 1) {
      setCurrentQuestion(prev => prev + 1);
      setSelectedAnswer(null);
      setIsAnswered(false);
    } else {
      setIsFinished(true);
      if (score >= gerechtigkeitQuiz.length - 2) {
  ...
  if (isFinished) {
    const percent = Math.round((score / gerechtigkeitQuiz.length) * 100);
    return (
      <div className="px-4 md:px-6 lg:px-8 py-4 max-w-2xl mx-auto text-center">
        ...
        <h1 className="text-2xl font-bold mb-2">{score} von {gerechtigkeitQuiz.length} richtig!</h1>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-left">
              ...
              <p className="text-white/90 text-sm">
                {percent >= 80 
                  ? "Super! Du verstehst die Dimensionen der Gerechtigkeit richtig gut!" 
                  : percent >= 60
                  ? "Gut gemacht! Schau dir die 4 Dimensionen nochmal an."
                  : "Nicht aufgeben! Lies nochmal durch und versuch es erneut."}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <Link href="/leni/sozialkunde/gerechtigkeit">
  ...
  return (
    <div className="px-4 md:px-6 lg:px-8 py-4 max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <Link href="/leni/sozialkunde/gerechtigkeit" className="flex items-center gap-2 text-gray-500 hover:text-gray-700">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <span className="font-medium text-gray-700">Frage {currentQuestion + 1}/{gerechtigkeitQuiz.length}</span>
      ...
      <div className="h-1.5 bg-gray-200 rounded-full mb-6 overflow-hidden">
        <div 
          ...
          style={{ width: `${((currentQuestion + 1) / gerechtigkeitQuiz.length) * 100}%` }}
        />
      </div>
      ...
      {isAnswered && (
        <button
          ...
          className="w-full py-4 bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:shadow-lg transition-shadow"
        >
          {currentQuestion < gerechtigkeitQuiz.length - 1 ? 'Weiter' : 'Ergebnis'}
